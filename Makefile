...
SOURCES = src/main.c src/history.c src/parents.c

.PHONY: all universal test clean
all: $(BIN)

$(BIN): $(SOURCES) src/history.h src/parents.h Makefile
	mkdir -p dist
	$(CC) $(CFLAGS) $(SOURCES) $(LDLIBS) -o $@

universal: $(SOURCES) src/history.h src/parents.h Makefile
	mkdir -p dist
...
test: all
	...
	./dist/history-test
	$(CC) $(CFLAGS) tests/parents_test.c src/parents.c -o dist/parents-test
	./dist/parents-test
